<template>
  <div class="favorite-container">
    ...
    <div class="question-list" v-loading="loading">
      ...
      <el-empty v-if="!loading && questions.length === 0" description="暂无收藏题目" />
    </div>
    ...
    <el-dialog v-model="dialogVisible" title="题目详情" width="800px">
      ...
      <template #footer>
        <el-button @click="removeFavorite" type="danger">移除收藏</el-button>
...
const loadQuestions = async () => {
  ...
  try {
    ...
    console.log('加载收藏题目，userID:', userID, 'category:', category)
    
    const result = await FavoriteService.GetFavoriteQuestionsByCategory(userID, category)
    console.log('收藏题目原始数据:', result)
    console.log('收藏题目数量:', result.length)
    ...
    console.error('Load favorite questions error:', error)
    ElMessage.error('加载收藏题目失败：' + (error.message || '未知错误'))
  } finally {
    loading.value = false
  }
}
...
const revealAnswer = () => {
  ...
  if (!isCorrect) {
    ...
    // 未答题也视同答错，需要加入错题本
    FavoriteService.AddToErrorBook(userStore.currentUser!.user_id, currentQuestion.value!.id, userStore.currentCategory)
      .catch(err => {
...
const removeFavorite = async () => {
  try {
    ...
    if (currentIndex.value >= totalQuestions.value) {
      currentIndex.value = Math.max(0, totalQuestions.value - 1)
    }
  } catch (error: any) {
